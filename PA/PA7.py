...
def insert_floor(floor_tree,floor):
    ...
    while x != None:
        y = x
        if floor.floor_num <x.floor_num:
            x = x.left
        else:
            x = x.right
    floor.parent = y
    ...
    elif floor.floor_num< y.floor_num:
        y.left = floor
    else:
        y.right = floor
...
def transplant(floor_tree,floor1,floor2):
    if floor1.parent == None:
        floor_tree.root = floor2
    elif floor1 == floor1.parent.left:
        floor1.parent.left = floor2
    else:
        floor1.parent.right = floor2
    if floor2 != None:
        floor2.parent = floor1.parent
def tree_mini(floor):
    while floor.left != None:
        floor = floor.left
    return floor
def delete_floor(floor_tree,floor):
    if floor.left == None:
        transplant(floor_tree,floor,floor.right)
    elif floor.right == None:
        transplant(floor_tree,floor,floor.left)
    else:
        y = tree_mini(floor.right)
        if y.parent !=floor:
            transplant(floor_tree,y,y.right)
            y.right = floor.right
            y.right.parent = y
        transplant(floor_tree,floor,y)
        y.left = floor.left
        y.left.parent = y
...
def search(floor_tree,num):
    x = floor_tree.root
    while x.left != None and x.right != None:
        if x.floor_num < num:
            x = x.right
        elif x.floor_num> num:
            x = x.left
        else:
            break
    return x
def find_nearest_unhaunted(floor_tree,floor_num):
    x = search(floor_tree,floor_num)
    if x.parent != None and x.left != None and x.right != None:
        pd = abs(x.parent.floor_num - floor_num)
        ld = abs(x.left.floor_num - floor_num)
        rd = abs(x.right.floor_num - floor_num)
        result = x
        if pd <ld:
            if pd<=rd:
                result = x.parent
        ...
        ld = abs(x.left.floor_num - floor_num)
        if x.right !=None:
            rd = x.right.floor_num - floor_num
            if rd < ld:
                result = x.right
    elif x.right != None:
        result = x.right
        ...
        result = None
    d = abs(x.floor_num-floor_num)
    if d<abs(result.floor_num-floor_num):
        result = x
    return result
