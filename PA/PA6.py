...
def database_insert(database,robot):
    database.name_table[hash_function(robot.name)].append(robot)
    database.shape_table[hash_function(robot.shape)].append(robot)
    database.color_table[hash_function(robot.color)].append(robot)
...
def database_search(database,field,value):
    ...
    result = []
    if field == "name":
        for x in database.name_table[i]:
            if x.name ==value:
                result.append(x)
    elif field == "shape":
        for x in database.shape_table[i]:
            if x.shape ==value:
                result.append(x)
    else:
        for x in database.color_table[i]:
            if x.color ==value:
                result.append(x)
    return result
...
def database_delete(database,field,value):
    ...
    x= 0
    if field == "name":
        while x < len(database.name_table[i]):
            if value == database.name_table[i][x].name:
                j = hash_function(database.name_table[i][x].color)
                database.color_table[j] = delete(database.color_table[j],"color",database.name_table[i][x])
                k = hash_function(database.name_table[i][x].shape)
                database.shape_table[k] = delete(database.shape_table[k],"shape",database.name_table[i][x])
                del database.name_table[i][x]
            else:
                x+=1
    elif field == "shape":
        while x < len(database.shape_table[i]):
            if value == database.shape_table[i][x].shape:
                j = hash_function(database.shape_table[i][x].color)
                database.color_table[j] = delete(database.color_table[j],"color",database.shape_table[i][x])
                k = hash_function(database.shape_table[i][x].name)
                database.name_table[k] = delete(database.name_table[k],"name",database.shape_table[i][x])
                del database.shape_table[i][x]
            else:
                x+=1
    else:
        while x < len(database.color_table[i]):
            if value == database.color_table[i][x].color:
                j = hash_function(database.color_table[i][x].shape)
                database.shape_table[j]= delete(database.shape_table[j],"shape",database.color_table[i][x])
                k = hash_function(database.color_table[i][x].name)
                database.name_table[k] = delete(database.name_table[k],"name",database.color_table[i][x])
                del database.color_table[i][x]
            else:
                x+=1
def delete(tableList,field,r):
    i = len(tableList)-1
    while i >=0:
        if tableList[i] == r:
            del tableList[i]
