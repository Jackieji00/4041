...
def insert_task(task_heap,task):
    task_heap.append(Task(" ",float("-inf")))
    task_heap[len(task_heap)-1] = task
    i = len(task_heap)-1
    while i>0 and task_heap[(i-1)//2].priority<task_heap[i].priority:
        tmp = task_heap[i]
        task_heap[i] = task_heap[(i-1)//2]
        task_heap[(i-1)//2] = tmp
        i = (i-1)//2
...
def max_heapify(A,i):
    ...
    if left<len(A) and A[left].priority>A[i].priority:
        ...
        max = i
    if right<len(A) and A[right].priority>A[i].priority:
        max = right
    if max!=i:
        tmp = A[i]
        A[i]= A[max]
...
def increase_task_priority(task_heap,i,new_priority):
    if task_heap[i].priority >=new_priority:
        return None
