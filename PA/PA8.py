...
def check(point,pt,distance,nearest):
    if pt == None:
    ...
    while pt != None:
        ...
        if y.depth %2 == 0:
            ...
            if point.y < pt.y :
                ...
                pt = pt.right
        #compare and update
        if distance > point.dist(y):
            nearest = y
            distance = point.dist(y)
        # print(y,nearest)
    # print(ptLst)
    # go back and check the point we have gone through
    while len(ptLst) != 0:
        p = ptLst[len(ptLst)-1]
        # print("----","p:",p,"pd:",point.dist(y))
        # print("in",p,abs(p.y-point.y) <= distance)
        if p.depth %2 == 0 and abs(p.x-point.x) <= distance:
            if point.x<=p.x:
                nearest,distance = check(point,p.right,distance,nearest)
            else:
                nearest,distance = check(point,p.left,distance,nearest)
        elif p.depth %2 == 1 and abs(p.y-point.y) <= distance:
            if point.y<=p.y:
                nearest,distance = check(point,p.right,distance,nearest)
            else:
                nearest,distance = check(point,p.left,distance,nearest)
        del ptLst[len(ptLst)-1]
    return nearest,distance
def nearest_neighbor(kd_tree,point):
    #search the nearest point on one branch
    root = kd_tree.root
    if root == None: #check if empty
        return None
    nearestPt = root
    distance = point.dist(nearestPt)
    nearestPt,distance = check(point,root,distance,nearestPt)
    return nearestPt
...
corrKD_6 = construct_test([None, (127, 197), (119, 157), (150, 119), (-13,-35),
                           (38, 158), (141, -193), (244, 182), (-87, 15), (99, -171),
